...
function doubleValues(arr){
   const timesTwo = arr.map(function(value){
    return value * 2;
})
...
const firstLast = arr.map(function(value){
    let firstWord = value.charAt(0);
    let lastWord = value.slice(-1);
...
function find(arr, searchValue) {
    ...
    let newArr = arr.find(ele => ele === searchValue);
    return newArr;
}
...
function findInObj(arr, key, searchValue) {
    let newArr= arr.find(ele => ele[key] === searchValue);
    return newArr;
}
